...
class Deposit extends Model
{
     ...
     * @return mixed
     */
    public static function frontPage($user)
    {
        return self::where('user_id','=',$user->id)->orderBy('created_at', 'desc')->paginate(config('marketplace.products_per_page'));
    }
     ...
     * @return mixed
     */
    public static function history($coin)
    {
        $user = auth()->user();
    ...
    /**
     * Return recent for front page
     *
     * @return mixed
     */
    public static function mostRecent($coin)
    {
        $user = auth()->user();
    ...
    {
        if ($this->status) {
            return base64_decode($this->address);
        } else {
            return $this->address;
        }
    }
